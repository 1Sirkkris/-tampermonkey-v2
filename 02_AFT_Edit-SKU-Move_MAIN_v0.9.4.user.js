// ==UserScript==
// @name         MAIN v0.9.4 AFT Edit/SKU/Move master
...
(() => {
  ...
  if (window.__AFT_MASTER_V094__) return;
  window.__AFT_MASTER_V094__ = true;
  ...
  function makeApi(instructionId, tool, stopped = () => false) {
    ...
    const wait = async (objectId, label, opts = {}) => {
      ...
      while (performance.now() - started < timeout) {
        ...
        catch { throw new Error(`${label}: invalid /status JSON`); }
        ...
        if (state === 'ERRORED') throw new Error(`${label}: backend ERRORED`);
        if (state !== 'PROCESSING') throw new Error(`${label}: status ${state || 'blank'}`);
  ...
  const Edit = {
    ...
    currentContainerMatches(target) {
      const wanted = low(target);
      return !!wanted && low(nativeText()).includes(wanted);
    },

    async acquireEachObject(oldObjectId = null, targetLocation = '') {
      let last = null;
      let avoidObjectId = oldObjectId;

      try {
        const domState = this.currentDocumentState();
        const domObject = currentObjectId();

        if (
          domObject &&
          (!avoidObjectId || domObject !== avoidObjectId) &&
          (domState === 'location' || domState === 'item')
        ) {
          const contextOk =
            domState === 'location' ||
            this.currentContainerMatches(targetLocation);

          if (contextOk) {
            try {
              const ready = await EditApi.wait(domObject, 'Each DOM start');
      ...
      for (let attempt = 1; attempt <= 10; attempt++) {
        ...
        if (avoidObjectId && snap.objectId === avoidObjectId) {
          await sleep(40);
          continue;
        }

        const ready = await EditApi.wait(
          snap.objectId,
          `Each start ${attempt}`
        );
    ...
    async runEachQueue(items, meta) {
      ...
      for (let i = 0; i < items.length; i++) {
        ...
        if (session.open && low(session.location) === low(item.location)) {
          ...
          if (session.open && session.objectId) {
            ...
            await EditApi.end(session.objectId);
            oldObjectId = session.objectId;
          }
          const fresh = await this.acquireEachObject(oldObjectId, item.location);
          session = { objectId: fresh.objectId, location: item.location, open: false };
          startState = fresh.startState;
        }

        const result = await this.runEachOne(session.objectId, item, meta, i + 1, items.length, startState);
        session = { objectId: result.objectId, location: result.location, open: result.workflowOpen };
        done++;
        const avg = (performance.now() - started) / done;
        this.status(`${done}/${items.length} done • last ${(result.elapsed / 1000).toFixed(2)}s • avg ${(avg / 1000).toFixed(2)}s`);
      }
      ...
      this.status(`DONE ✓ ${done}/${items.length} • avg ${(total / done / 1000).toFixed(2)}s`);
    },
