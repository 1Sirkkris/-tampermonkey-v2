// ==UserScript==
// @name         TEST v0.2.3 FCR Data Core
...
(() => {
  ...
  if (window.__fcrDataCore_v023test) return;
  window.__fcrDataCore_v023test = true;
  ...
  async function fcrPost(endpoint, searchValue) {
    const search = clean(searchValue);
    const key = `fcr:${endpoint}:${upper(search)}`;
    ...
    const work = (async () => {
      ...
      try {
        const response = await fetch(`${apiBase()}/${endpoint}`, {
          ...
          body: new URLSearchParams({ s: search }).toString(),
        ...
        if (error?.name === 'AbortError') throw new Error(`${endpoint}: timed out`);
        throw error;
      } finally {
        clearTimeout(timer);
      }
  ...
  async function fetchInventory(container) {
    const search = clean(container);
    if (!search) return { rows: [], source: 'empty', ms: 0 };

    // Never cache tote inventory
    stats.inventoryNetwork++;
    const response = await fcrPost('inventory', search);
    return {
      rows: parseInventoryHtml(response.text),
      source: 'network',
      ms: response.ms
    };
  }
  ...
  async function fetchProduct(code, required = []) {
    const search = clean(code);
    ...
    const key = `product:${upper(search)}`;
    if (inFlight.has(key)) {
      stats.dedupeHits++;
    ...
    const work = (async () => {
      ...
      const response = await fcrPost('product', search);
  ...
  async function handleRequest(message) {
    const { id, type, payload = {}, client = 'unknown' } = message || {};
    if (!id || !type) return;
    ...
    try {
      let data;
      if (type === 'ping') data = { version: VERSION, modules: ['product', 'inventory', 'history', 'hazmat', 'binSize'], stats: { ...stats } };
      else if (type === 'inventory') data = await fetchInventory(payload.container || payload.code);
      ...
      else if (type === 'product') data = await fetchProduct(payload.code, Array.isArray(payload.require) ? payload.require : []);
