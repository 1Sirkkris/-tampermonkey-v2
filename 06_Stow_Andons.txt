// ==UserScript==
// @name         TEST v5.4.10 Stow Andons Helper — Safe Trim
...
// @connect      localhost
// @version      5.4.10
// @description  TEST: Clean Safe Trim using shared FCR Data Core.
...
(() => {
  ...
  async function scanSussy(containerPage) {
    ...
    await Promise.all([...groups.entries()].map(async ([code, entries]) => {
