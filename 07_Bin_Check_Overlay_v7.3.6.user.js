// ==UserScript==
// @name         TEST v7.3.6 Bin check Overlay
// @namespace    https://gist.github.com/1Sirkkris
// @version      7.3.6
// @description  TEST: Clean native bin overlay with floor filters, quantity sorting and lazy P2/P3/P4 copy.
...
(() => {
  ...
  if (window.__binOverlay_v736test || location.hash.startsWith("#fcr-tote-checker")) return;
  window.__binOverlay_v736test = true;
  ...
  function installStartButton() {
    ...
    if (!table || !nav) return false;
    if (document.getElementById('p-level-overlay-start-btn')) return true;
    ...
    Object.assign(button.style, {
      order: '5', backgroundColor: 'orange', color: 'black', marginLeft: '8px', padding: '6px 10px',
      borderRadius: '6px', fontWeight: '700', cursor: 'pointer', border: '1px solid #92400E'
    });
  ...
  function fetchPodFloor(pod) {
    ...
    fetch(hierarchyUrl(pod), { credentials: 'same-origin', cache: 'no-store', signal: controller.signal })
      .then(response => {
    ...
    overlay.innerHTML = `
      <div id="pLevelOverlayHeader">
        <div id="pLevelOverlaySummary">
          <span id="pLevelOverlayTitle">Overlay v7.3.4</span>
      ...
      <div id="pLevelOverlayBody">
        ...
        <div class="p-level-muted" id="pLevelOverlayHint">Floor + quantity filters. FCResearch table unchanged.</div>
