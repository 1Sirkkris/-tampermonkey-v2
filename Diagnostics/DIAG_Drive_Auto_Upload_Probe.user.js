// ==UserScript==
// @name         DIAG v0.2.0 Drive Auto Upload Probe
// @namespace    MONKIES
// @version      0.2.0
...
(() => {
  ...
  const STORE_KEY = 'bwu2DriveAutoUploadTarget_v2';
  ...
  function installCapture() {
    ...
    if (!NativeXHR || NativeXHR.prototype.__bwu2DriveAutoUploadCapture) return;

    const nativeOpen = NativeXHR.prototype.open;
    const nativeSend = NativeXHR.prototype.send;

    Object.defineProperty(NativeXHR.prototype, '__bwu2DriveAutoUploadCapture', {
      value: true,
      configurable: true,
    });
  ...
  function updateStatus() {
    ...
    setStatus(
      target
        ? 'TARGET CAPTURED\nReady to test Drive same-origin upload.'
  ...
  function buildPanel() {
    ...
    run.id = `${PANEL_ID}-run`;
    run.type = 'button';
    run.textContent = 'TEST DRIVE UPLOAD';
    run.addEventListener('click', runUploadProbe);
