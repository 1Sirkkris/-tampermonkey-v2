// ==UserScript==
// @name         MAIN v0.2.1 Sideline API Move TEST
...
`;
  ...
  function installContainerAdvance(input, next) {
  ...
  installContainerAdvance(lSrc, lDest);
  ...
  lazyPanel.onclick = e => {
    ...
    if (a === 'toggle-items') {
      lazy.inputCollapsed = !lazy.inputCollapsed;
      renderLazy();
      return;
    }
    ...
    if (a === 'pause') {
      ...
      e.target.textContent = lazy.paused ? 'Resume' : 'Pause';
