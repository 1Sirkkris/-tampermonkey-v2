...
| FCResearch → RIVER Ticket Assistant | 0.3.3 | Event-driven Hazmat N/A/L0 capture: reuse loaded FCResearch data first, fall back only for still-missing PO/vendor or quantity, bind Vendor Code to the selected latest PO row, then run the approved RIVER sequence with the required manual checkpoint. | [Open](https://raw.githubusercontent.com/1Sirkkris/-tampermonkey-v2/main/FCResearch_RIVER_Ticket_Assistant.user.js) |