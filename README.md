...
| Sideline API Move | 0.3.7 | [Open](https://raw.githubusercontent.com/1Sirkkris/-tampermonkey-v2/main/Sideline_API_Move.user.js) |
...
| FCResearch → RIVER Ticket Assistant | 0.3.8 | Event-driven Hazmat N/A/L0 capture. RIVER step recognition now uses the workflow-generated `page-info` label (for example `PANDASH CHECK`) as the primary state marker, with one bounded DOM observer only while waiting for a real transition. Latest matching PO/vendor/quantity rules remain unchanged. | [Open](https://raw.githubusercontent.com/1Sirkkris/-tampermonkey-v2/main/FCResearch_RIVER_Ticket_Assistant.user.js) |
