...
test('unified diagnostic redacts sensitive values and fingerprints identifiers', async () => {
  ...
  const raw = {
    ...
    nested:{ email:'person@example.invalid', note:'Pod P-2-A123B456 and item X0ABCDEFGH' },
    url:'https://example.invalid/api/test?token=SECRET-TOKEN&mode=view&s=tsXPRIVATE123'
  };
  const sanitized = sanitize(raw);
  const output = JSON.stringify(sanitized);
  for (const secret of ['SECRET-TOKEN','tsXPRIVATE123','OBJECT-PRIVATE-456','person@example.invalid','P-2-A123B456','X0ABCDEFGH']) {
